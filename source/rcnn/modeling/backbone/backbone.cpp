...
Backbone BuildResnetFpn_p3p7_backbone()
{
  ...
  int64_t in_channels_stage2 = rcnn::config::GetCFG<int64_t>({"MODEL", "RESNETS", "RES2_OUT_CHANNELS"});
  int64_t out_channels = rcnn::config::GetCFG<int64_t>({"MODEL", "RESNETS", "BACKBONE_OUT_CHANNELS"});
  int64_t in_channels_p6p7 = rcnn::config::GetCFG<bool>({"MODEL", "RESNETS", "USE_C5"}) ? in_channels_stage2 * 8 : out_channels;
  model->push_back("body", body);
  FPNLastLevelP6P7 fpn = FPNLastLevelP6P7(rcnn::config::GetCFG<bool>({"MODEL", "FPN", "USE_RELU"}),
                                          std::vector<int64_t>{ in_channels_stage2 * 2, in_channels_stage2 * 4, in_channels_stage2 * 8},
                                          in_channels_p6p7, out_channels, rcnn::layers::ConvWithKaimingUniform);
  model->push_back("fpn", fpn);
  auto backbone = Backbone(model, out_channels);
  return backbone;
}
} // namespace modeling
