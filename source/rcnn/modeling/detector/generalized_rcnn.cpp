...
GeneralizedRCNNImpl::GeneralizedRCNNImpl()
    : backbone(register_module("backbone", BuildBackbone())),
      ...
      fcos(register_module("fcos", FCOSModule(backbone->get_out_channels())))
{
  use_fcos = rcnn::config::GetCFG<bool>({"MODEL", "FCOS_ON"});
}
...
std::vector<rcnn::structures::BoxList> GeneralizedRCNNImpl::forward(std::vector<torch::Tensor> images)
{
  ...
  if (use_fcos)
  {
    std::tie(proposals, proposal_losses) = fcos->forward(imageList, features);
  }
  else
  {
